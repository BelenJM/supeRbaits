...
main_function <- function(n, size, database, exclusions = NULL, 
  ...
  on.exit(save(list = ls(), file = "supeRbaits_debug.RData"), add = TRUE)
	...
	if (!is.null(regions.prop) && (!is.numeric(regions.prop) | (regions.prop > 100 | regions.prop < 1)))
		stop("'regions.prop' must be numeric and between 1 and 100.\n")
	if (!is.null(targets.prop) && (!is.numeric(targets.prop) | (targets.prop > 100 | targets.prop < 1)))
		stop("'targets.prop' must be numeric and between 1 and 100.\n")
